...
//	private int nadc;
	...
	@Override
	public byte[] decode(byte[] bs, final int offset, final int length) {
		int off;
		
		/*
		 * part1 : 帧边信息
		 */
		int i = getSideInfo(bs, offset); // 帧边信息占的 byte 数
		if (i < 0)
			return null; // 跳过这一帧的主数据
		off = i;
	...
	/**
	 * 处理帧边信息, 修改 infoCh0 和 infoCh1 中的数据
	 * @param b
	 * @param off
	 * @return
	 *   处理帧边信息之后, 指向音频主数据的索引
		...
		for (gr = 0; gr < ngr; gr++) {
			...
			for (ss = 0; ss < 18; ss += 2) {
				for (i = ss, sb = 0; sb < 32; sb++, i += 18)
					samples[sb] = b[i];
				synthesisSubBand(samples, ch); //多相合成滤波
	...
	private class ChannelInformation {
		// 从位流读取数据按下列顺序依次初始化的 14 个变量, 见 getSideInfo 方法.
